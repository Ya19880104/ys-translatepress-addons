...
## [0.11.2] - 2026-06-25 — 列表語言篩選標籤優化

### 變更
- 列表語言篩選下拉的選項標籤加上「顯示於／隱藏於」前綴（如「顯示於 English」），下拉收合後仍可清楚辨識，避免兩組同名語言混淆。

## [0.11.1] - 2026-06-25 — 列表語言篩選

### 新增
- **列表語言篩選** — 於文章／頁面／自訂內容列表上方新增「全部語言」下拉篩選，可篩「顯示於／隱藏於」某語言，快速找出各語言可見（或被排除）的內容。

## [0.11.0] - 2026-06-25 — 列表語言可用性欄

### 新增
- **列表語言欄** — 於文章／頁面／自訂內容的後台列表加入「語言」欄，以小國旗＋綠勾顯示各語言是否顯示（依「內容語言規則」，被排除的語言呈灰階），一眼掌握每筆內容的多語可用性。於「內容語言規則」設定頁以「列表顯示語言欄」開關控制（預設開啟）。
